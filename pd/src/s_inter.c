...
double sys_getrealtime(void) {
    ...
    if (INTER->i_freq == 0)
        sys_initntclock();
    return (((double)(now.QuadPart - INTER->i_inittime.QuadPart)) /
            INTER->i_freq);
#endif
}
...
void sys_set_priority(int mode) {
    ...
    if (sched_setscheduler(0, (mode == MODE_NRT ? SCHED_OTHER : SCHED_FIFO),
                           &par) < 0) {
        ...
        if (mode == MODE_RT)
            ...
            post("running at normal (non-real-time) priority.\n");
    }
...
// #define GUI_BYTESPERPING 0x7fffffff /* as per Miller's suggestion to disable
// the flow control */
...
static void sys_trytogetmoreguibuf(int newsize) {
    ...
    if (!newbuf) {
        ...
        int written = 0;
        while (1) {
            int res = do_send(INTER->i_guisock,
...
void sys_vvgui(const char *fmt, va_list ap) {
    ...
    if (sys_debuglevel & DEBUG_MESSUP) {
        ...
        if (stderr_isatty)
            fprintf(stderr, "%s\e[0;1;35m%s\e[0m", begin ? "\n-> " : "",
    ...
    if (INTER->i_guihead > 0)
        lastend = INTER->i_guibuf[INTER->i_guihead - 1];
    if (INTER->i_guihead > 1 &&
        strcmp(INTER->i_guibuf + INTER->i_guihead - 2, "\\\n") == 0)
        lastend = ' ';
}
#undef sys_vgui
#undef sys_gui
...
void gui_do_vmess(const char *sel, char *fmt, int end, va_list ap) {
    ...
    while (*fp) {
        ...
        switch (*fp++) {
        case 'f':
            sys_vgui(FLOAT_SPECIFIER, va_arg(ap, double));
            break;
        case 's':
            escape_double_quotes(va_arg(ap, const char *));
            break;
        case 'i':
            sys_vgui("%d", va_arg(ap, int));
            break;
        case 'x':
            sys_vgui("\"" X_SPECIFIER "\"", va_arg(ap, t_uint));
            break;
...
void sys_vguid(const char *file, int line, const char *fmt, ...) {
    ...
    gui_vmess("gui_legacy_tcl_command", "sis", file, line, bufp);
...
int sys_flushtogui(void) {
    ...
    if (writesize > 0)
        nwrote = do_send(INTER->i_guisock, INTER->i_guibuf + INTER->i_guitail,
                         writesize, 0);
...
static int sys_do_startgui(const char *guidir) {
    ...
    if (sys_guisetportnumber) /* GUI exists and sent us a port number */
    {
        ...
        status = addrinfo_get_list(&ailist, LOCALHOST, sys_guisetportnumber,
                                   SOCK_STREAM);
        if (status != 0) {
            fprintf(
                stderr,
    ...
    else /* default behavior: start up the GUI ourselves. */
    {
        ...
        if (sys_guicmd) {
            ...
            sprintf(cmdbuf,
                    ...
                    wish_paths[i], data_dir_flag, guidir2, portno,
                    (sys_k12_mode ? "pd-l2ork-k12" : "pd-l2ork"), guidir2,
                    (t_uint)pd_this);
#else
            sprintf(
                cmdbuf,
            ...
            sprintf(cmdbuf,
                    "%s/nw/nw %s %s "
                    "%d localhost %s %s " X_SPECIFIER,
                    guidir2, data_dir_flag, guidir2, portno,
                    (sys_k12_mode ? "pd-l2ork-k12" : "pd-l2ork"), guidir2,
                    (t_uint)pd_this);
        ...
        spawnret = _spawnl(
            P_NOWAIT, wishbuf, "pd-nw", data_dir_flag, scriptbuf, portbuf,
            "localhost", (sys_k12_mode ? "pd-l2ork-k12" : "pd-l2ork"),
    ...
    if (!sys_nogui) {
        ...
        gui_start_vmess("gui_startup", "sss", pd_version, sys_font,
                        sys_fontweight);
...
void sys_bail(int n) {
    ...
    !defined(__GNU__) /* sys_close_audio() hangs if you're in a signal? */
