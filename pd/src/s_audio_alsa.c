    ...
    int nfrags, int frag_size)
{
    ...
    if (err < 0)
    {
        ...
        if (err < 0)
        {
            err = snd_pcm_hw_params_set_format(dev->a_handle, hw_params,
                SND_PCM_FORMAT_S16);
            check_error(err, out, "snd_pcm_hw_params_set_format");
            dev->a_sampwidth = 2;
        }
    ...
    int *choutdev, int rate, int blocksize)
{
    ...
    for (iodev = 0; iodev < naudioindev; iodev++)
    {
        ...
        check_error(err, 0, "snd_pcm_open (input)");
        if (err < 0)
            continue;
    ...
    for (iodev = 0; iodev < naudiooutdev; iodev++)
    {
        ...
        check_error(err, 1, "snd_pcm_open (output)");
        if (err < 0)
            continue;
...
void alsa_close_audio(void)
{
    ...
    for (iodev = 0; iodev < alsa_nindev; iodev++)
    {
        ...
        check_error(err, 0, "snd_pcm_close (input)");
    }
    for (iodev = 0; iodev < alsa_noutdev; iodev++)
    {
        err = snd_pcm_close(alsa_outdev[iodev].a_handle);
        check_error(err, 1, "snd_pcm_close (output)");
    }
    alsa_nindev = alsa_noutdev = 0;
}
...
int alsa_send_dacs(void)
{
    ...
    t_sample *fp1, *fp2;
    int i, j, k, iodev, result, ch, goterror = 0, busy = 0;
...
#endif
    /* do output */
    for (iodev = 0, fp1 = STUFF->st_soundout, ch = 0; iodev < alsa_noutdev; iodev++)
    {
...

    {
        ...
        {
            checkcountdown = 10;
            if (alsa_nindev > 0 && alsa_noutdev > 0 && !goterror)
...
static void alsa_checkiosync(void)
{
    int result, giveup = 50, alreadylogged = 0, iodev = 0, err;
    ...
    *canmulti = 2;  /* supports multiple devices */
    ...
    for (ndev = 0; ndev < alsa_nnames; ndev++)
    {
        ...
        snprintf(outdevlist + ndev * devdescsize, devdescsize, "%s",
            alsa_names[ndev]);
        indevlist[(ndev+1) * devdescsize - 1] =
            outdevlist[(ndev+1) * devdescsize - 1] = 0;
    }
    ...
    while (!snd_card_next(&cardno) && cardno >= 0)
    {
        ...
        if (snd_ctl_open(&ctl, devname, 0) >= 0)
        {
            ...
            snprintf(indevlist + ndev * devdescsize, devdescsize,
                "%s (hardware)", desc);
            snprintf(indevlist + (ndev + 1) * devdescsize, devdescsize,
                "%s (plug-in)", desc);
            snprintf(outdevlist + ndev * devdescsize, devdescsize,
                "%s (hardware)", desc);
            snprintf(outdevlist + (ndev + 1) * devdescsize, devdescsize,
                "%s (plug-in)", desc);
            indevlist[(ndev+1) * devdescsize - 1] =
                indevlist[(ndev+2) * devdescsize - 1] =
                    outdevlist[(ndev+1) * devdescsize - 1] =
                        outdevlist[(ndev+2) * devdescsize - 1] = 0;
        }
        else
        {
