...
static t_patchlist *patchlist_append(t_patchlist *listwas, const char *files,
                                     const char *args) {
    t_namelist *nl, *nl2;
...
static void openit(const char *dirname, const char *filename,
    ...
    int fd =
        open_via_path(dirname, filename, "", dirbuf, &nameptr, FILENAME_MAX, 0);
    if (fd >= 0) {
        sys_close(fd);
        if (args && *args) {
...
static int defaultfontshit[] = {8,  5,  11, 9,  6,  12, 10, 6,  13, 12,
                                7,  16, 14, 8,  17, 16, 10, 19, 18, 11,
                                22, 24, 14, 29, 30, 18, 37, 36, 22, 44};
...
int sys_main(int argc, const char **argv) {
    ...
    if (getuid() != geteuid()) {
        ...
        if (setuid(getuid()) < 0) {
            ...
            fprintf(stderr, "\nTo fix this, please remove the setuid flag from "
                            "the Pd binary");
            if (argc > 0) {
                fprintf(stderr,
...
// LATER: distinguish between WebPdL2Ork emscripten and full online editor
    ...
    gui_start_vmess("gui_set_browser_config", "iiiiii", sys_browser_doc,
                    sys_browser_path, sys_browser_init, sys_autocomplete,
                    sys_autocomplete_prefix, sys_autocomplete_relevance);
    gui_start_array();
...
static char *(usagemessage[]) = {
    ...
    "-nojackconnect   -- don't make any automatic connections to the jack "
    "graph\n",
    ...
    "-nostderr        -- send printout to GUI instead of standard error (true "
    "by default)\n",
    ...
    "-legacy-bendin   -- enable legacy (unsigned) bendin (disabled by "
    "default)\n",
    "\n",
};
...
int sys_argparse(int argc, const char **argv) {
    ...
    while ((argc > 0) && **argv == '-') {
        ...
        if (!strcmp(*argv, "-r") && argc > 1 &&
            ...
            if (argc < 2 ||
                !sys_parsedevlist(&as.a_nchindev, as.a_chindevvec,
                                  MAXAUDIOINDEV, argv[1]) ||
                !sys_parsedevlist(&as.a_nchoutdev, as.a_choutdevvec,
                                  MAXAUDIOINDEV, argv[1]))
                goto usage;
            argc -= 2;
            argv += 2;
            ...
            argc -= 2;
            argv += 2;
        } else if ((!strcmp(*argv, "-sounddev") ||
                    !strcmp(*argv, "-audiodev"))) {
            if (argc < 2 ||
                !sys_parsedevlist(&as.a_nindev, as.a_indevvec, MAXAUDIOINDEV,
                                  argv[1]) ||
                !sys_parsedevlist(&as.a_noutdev, as.a_outdevvec, MAXAUDIOOUTDEV,
                                  argv[1]))
                goto usage;
            argc -= 2;
            argv += 2;
        ...
        /* other flags */
            ...
            argc -= 2;
            argv += 2;
        } else if ((!strcmp(*argv, "-font-face") ||
                    !strcmp(*argv, "-typeface")) &&
                   argc > 1) {
            ...
            argv++;
                 ...
                 !strcmp(*argv, "-norealtime")) {
            fprintf(stderr,
                    "Pd-L2Ork compiled without realtime priority-support, "
                    "ignoring '%s' flag\n",
                    *argv);
            argc--;
            argv++;
