...
void outmidi_controlchange(int portno, int channel, int ctl, int value) {
    ...
    if (value < 0)
        ...
        value = 127;
    sys_queuemidimess(portno, 0, MIDI_CONTROLCHANGE + (channel & 0xf), ctl,
                      value);
}
...
void outmidi_programchange(int portno, int channel, int value) {
    if (value < 0)
        ...
        value = 127;
    sys_queuemidimess(portno, 0, MIDI_PROGRAMCHANGE + (channel & 0xf), value,
                      0);
}
...
void outmidi_polyaftertouch(int portno, int channel, int pitch, int value) {
    ...
    if (value < 0)
        ...
        value = 127;
    sys_queuemidimess(portno, 0, MIDI_POLYAFTERTOUCH + (channel & 0xf), pitch,
                      value);
}
...
static void sys_dispatchnextmidiin(void) {
    ...
    if (byte >= 0xf8) {
        ...
        if (byte & 0x80) {
            ...
            int cmd =
                (parserp->mp_status >= 0xf0 ? parserp->mp_status
...
void sys_get_midi_apis(char *buf) {
    ...
    strcpy(buf, "{ ");
#ifndef USEAPI_ALSA
    sprintf(buf + strlen(buf), "{OSS %d} ", API_DEFAULT);
    n++;
#else
    n++;
#endif
#ifdef USEAPI_ALSA
    sprintf(buf + strlen(buf), "{ALSA %d} ", API_ALSA);
    n++;
#endif
    strcat(buf, "}");
...
void sys_get_midi_params(int *pnmidiindev, int *pmidiindev, int *pnmidioutdev,
                         int *pmidioutdev) {
    int i, devn;
...
static void sys_save_midi_params(int nmidiindev, int *midiindev,
...
#ifdef USEAPI_ALSA
        if (sys_midiapi == API_ALSA)
            sys_alsa_do_open_midi(nmidiindev, midiindev, nmidioutdev,
                                  midioutdev);
        else
#endif /* ALSA */
...
#ifdef USEAPI_ALSA
    if (sys_midiapi == API_ALSA)
        midi_alsa_getdevs(indevlist, &nindevs, outdevlist, &noutdevs, MAXNDEV,
                          DEVDESCSIZE);
    else
#endif /* ALSA */
        midi_getdevs(indevlist, &nindevs, outdevlist, &noutdevs, MAXNDEV,
                     DEVDESCSIZE);
...
void glob_midi_properties(t_pd *dummy, t_floatarg flongform) {
    ...
    int midiindev1, midiindev2, midiindev3, midiindev4, midiindev5, midiindev6,
        midiindev7, midiindev8, midiindev9, midiindev10, midioutdev1,
        midioutdev2, midioutdev3, midioutdev4, midioutdev5, midioutdev6,
        midioutdev7, midioutdev8, midioutdev9, midioutdev10;

    /* these are all the devices on your system: */
    char indevlist[MAXNDEV * DEVDESCSIZE], outdevlist[MAXNDEV * DEVDESCSIZE];
    int nindevs = 0, noutdevs = 0, i;

    sys_get_midi_devs(indevlist, &nindevs, outdevlist, &noutdevs, MAXNDEV,
                      DEVDESCSIZE);
...
#ifdef USEAPI_ALSA
    if (sys_midiapi == API_ALSA) {
        midi_alsa_getdevs(indevlist, nindevs, outdevlist, noutdevs, maxndevs,
                          devdescsize);
    } else
#endif /* ALSA */
        midi_getdevs(indevlist, nindevs, outdevlist, noutdevs, maxndevs,
                     devdescsize);
}
