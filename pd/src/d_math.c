...
static void sigrsqrt_dsp(t_sigrsqrt *x, t_signal **sp)
{
    t_int n = SIGTOTAL(sp[0]);
    signal_setmultiout(&sp[1], sp[0]->s_nchans);
    dsp_add(((n & 7) ? sigrsqrt_perform : sigrsqrt_perform8), 3, sp[0]->s_vec, sp[1]->s_vec, n);
}
...
static void sigsqrt_dsp(t_sigsqrt *x, t_signal **sp)
{
    t_int n = SIGTOTAL(sp[0]);
    signal_setmultiout(&sp[1], sp[0]->s_nchans);
    dsp_add(((n & 7) ? sigsqrt_perform : sigsqrt_perform8), 3, sp[0]->s_vec, sp[1]->s_vec, n);
}
...
static void sigwrap_dsp(t_sigwrap *x, t_signal **sp)
{
    ...
    if (pd_compatibilitylevel < 48)
        dsp_add(sigwrap_perform_old, 3, sp[0]->s_vec, sp[1]->s_vec, n);
