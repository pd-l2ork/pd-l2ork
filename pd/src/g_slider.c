...
static void slider_interactive(t_slider *x, t_floatarg f);
...
static void slider_draw_new(t_slider *x, t_glist *glist)
{
    ...
    if (x->x_orient) {
        gui_vmess("gui_slider_new", "xxsiiiiii",
            canvas, x,
            x->x_gui.x_fcol->s_name, x1+2, r, x2-2, r, x1, y1);
    } else {
        gui_vmess("gui_slider_new", "xxsiiiiii",
            canvas, x,
            x->x_gui.x_fcol->s_name, r, y1+2, r, y2-2, x1, y1);
    }
}
...
static void slider_draw_config(t_slider *x, t_glist *glist)
{
    ...
    gui_vmess("gui_slider_indicator_color", "xxs",
        canvas, x, x->x_gui.x_fcol->s_name);
}
...
static void slider_save(t_gobj *z, t_binbuf *b)
{
    ...
    binbuf_addv(b, "ssiisiiffiisssiiiisssiiii", gensym("#X"),gensym("obj"),
        ...
        x->x_val, x->x_steady, x->x_exclusive, x->x_gui.x_click);
    binbuf_addv(b, ";");
}
...
static void slider_dialog(t_slider *x, t_symbol *s, int argc, t_atom *argv)
{
    ...
    slider_interactive(x, atom_getfloatarg(20, argc, argv));
    if (x->x_orient) {
    ...
    t_floatarg shift, t_floatarg ctrl, t_floatarg alt)
{
    if (x->x_gui.x_click)
    {
        ...
        glist_grab(x->x_gui.x_glist, &x->x_gui.x_obj.te_g,
            (t_glistmotionfn)slider_motion, 0, (t_glistkeynameafn)slider_list, xpos, ypos, x->x_exclusive);
    }
}
...
//        bflcol[2] = (int)atom_getintarg(15, argc, argv);
    ...
    if (argc >= 20&&IS_A_FLOAT(argv,19))
            x->x_gui.x_click = atom_getintarg(19, argc, argv);
...
void slider_addmethods(t_class *c) {
    ...
    class_addmethod(c, (t_method)slider_interactive, gensym("interactive"),
        A_FLOAT, 0);
