...
static void ptrobj_delete(t_ptrobj *x)
{
    ...
    // This was from pure-data, but I'm not sure if it's needed, because this member doesn't exist in pdl2ork
    // if (gp->gp_un.gp_gobj->sc_template == gensym("pd-text"))
    // {
    //     pd_error(x, "pointer delete: can't delete 'pd-text' scalar");
    //     return;
    // }
    // if (gp->gp_un.gp_gobj->sc_template == gensym("pd-float-array"))
    // {
