...
static v8::MaybeLocal<v8::Value> js_marshal_atom(const t_atom* atom)
{
    ...
    if (type == A_FLOAT)
    {
        return v8::Number::New(js_isolate, atom_getfloat((t_atom*)atom));
    }
    else if (type == A_SYMBOL)
    {
        v8::Local<v8::String> s;
        if (v8::String::NewFromUtf8(js_isolate, atom_getsymbol((t_atom*)atom)->s_name).ToLocal(&s))
            return s;
    }
...
{
    string ps(atom_getsymbol((t_atom*)a)->s_name);
    try
    {
        auto p = stoull(ps);
...
static js_file js_getfile(const t_js *x, const char* script_name)
{
    ...
    if (fd < 0)
    {
        pd_error((void *)&x->x_obj, "Script file '%s' not found.", script_name);
        return result;
    }
...
static void js_anything(t_js_inlet* inlet, const t_symbol* s, int argc, const t_atom* argv)
{
    ...
    if (msgname == "compile")
    {
        if (argc > 0 && argv[0].a_type == A_SYMBOL)
        {
            ...
            js_set_inlets(x, 1);
            js_set_outlets(x, 1);
            js_load(x, atom_getsymbol((t_atom*)&argv[0])->s_name);
        }
        else
        {
            js_load(x);
